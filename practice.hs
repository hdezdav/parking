...
cargarParqueadero = do
    handle <- openFile "parqueadero.txt" ReadMode
    contenido <- hGetContents handle
    let lineas = lines contenido
    let parqueadero = map leerVehiculo lineas
    length parqueadero `seq` hClose handle  -- Forzar la evaluación para asegurar que el contenido se lea antes de cerrar el handle
    return parqueadero
    where
...
-- Función para mostrar los vehículos en el parqueadero en pantalla
...
-- Función para mostrar los vehículos en el parqueadero y guardarlos en un archivo de texto
...
cicloPrincipal parqueadero = do
    ...
    case opcion of
        ...
        "4" -> do
            mostrarYGuardarVehiculos parqueadero
            cicloPrincipal parqueadero
